...
/** Fetch notes for a subject (optionally filtered by chapter). */
export async function fetchNotes(subjectId: string, chapterId?: string): Promise<NoteDoc[]> {
  const base = collection(db, COLLECTIONS.NOTES);
  const q = chapterId
    ? query(base, where("subjectId", "==", subjectId), where("chapterId", "==", chapterId))
